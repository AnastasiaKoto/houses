...
class CBitrixCatalogSmartFilter extends CBitrixComponent
{
	...
	// модифицирована для раздела с вариативными проектами
	public function getResultItems()
	{
		// Если это специальный раздел, используем свойства связанного инфоблока
		if ($this->isSpecialSection()) {
			$items = $this->getLinkedIblockItems();
			$this->arResult["PROPERTY_COUNT"] = count($items);
			$this->arResult["PROPERTY_ID_LIST"] = array_keys($items);
			$this->arResult["IS_LINKED_FILTER"] = true; // Флаг для шаблона
		} else {
			// Стандартная логика
			$items = $this->getIBlockItems($this->IBLOCK_ID);
			$this->arResult["PROPERTY_COUNT"] = count($items);
			$this->arResult["PROPERTY_ID_LIST"] = array_keys($items);
			$this->arResult["IS_LINKED_FILTER"] = false;
	...
	*/
	...
	//проверяем на секцию только для секции с вариативными проектами и дочерних
	protected function isSpecialSection()
	{
		$specialSectionId = (int)$this->arParams['SPECIAL_SECTION_ID'];
		if (!$specialSectionId) {
			return false;
		}
		
		if ($this->SECTION_ID === $specialSectionId) {
			return true;
		}
		
		// Проверяем дочерние разделы
		$rsSection = CIBlockSection::GetList(
			array(),
			array('ID' => $this->SECTION_ID, 'IBLOCK_ID' => $this->IBLOCK_ID),
			false,
		...
		if ($arSection = $rsSection->Fetch()) {
			...
			return $parentId === $specialSectionId;
		}
	...
	//получаем свойства предложений
	protected function getLinkedIblockProperties()
	{
		$linkedIblockId = (int)$this->arParams['LINKED_IBLOCK_ID'];
		if (!$linkedIblockId) {
			return array();
		}
	...
	protected function getLinkedIblockItems()
	{
		$items = array();
		$linkedProperties = $this->getLinkedIblockProperties();
		
		foreach ($linkedProperties as $arProperty) {
			$items[$arProperty["CODE"]] = array(
				"ID" => $arProperty["ID"],
				"IBLOCK_ID" => $arProperty["IBLOCK_ID"],
				"CODE" => $arProperty["CODE"],
				"~NAME" => $arProperty["NAME"],
				...
				"FILTER_HINT" => "",
				"VALUES" => array(),
				"ENCODED_ID" => md5($arProperty["ID"]),
				"IS_LINKED" => true, // Флаг что это свойство из связанного инфоблока
				"LINKED_IBLOCK_ID" => $this->arParams['LINKED_IBLOCK_ID'], // Добавляем ID связанного инфоблока
			);
		}
	...
	protected function processLinkedIblockValues(&$arResult)
	{
		$linkedIblockId = (int)$this->arParams['LINKED_IBLOCK_ID'];
		$linkPropertyCode = (string)$this->arParams['LINK_PROPERTY_CODE'];
		
		if (!$linkedIblockId || !$linkPropertyCode) {
			return;
		}
		...
		$elementIds = array();
		$rsElements = CIBlockElement::GetList(array('ID' => 'ASC'), $arElementFilter, false, false, array('ID'));
		...
		// Получаем связанные элементы из другого инфоблока
		$linkedElementIds = array();
		$rsLinked = CIBlockElement::GetList(
			array(),
			array(
				'IBLOCK_ID' => $linkedIblockId,
				'PROPERTY_' . $linkPropertyCode => $elementIds
			),
			false,
			false,
			array('ID')
		...
		// Собираем значения свойств из связанного инфоблока
		foreach ($arResult["ITEMS"] as $PID => $arItem) {
			if (!isset($arItem['IS_LINKED']) || !$arItem['IS_LINKED']) {
				continue;
			}
			...
			while ($arProperty = $rsProperties->Fetch()) {
				if ($arProperty['VALUE'] !== false && $arProperty['VALUE'] !== '') {
					$this->fillItemValues($arResult["ITEMS"][$PID], $arProperty['VALUE']);
				}
			}
		}
	}
